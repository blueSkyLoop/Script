...
#
path="/Users/zz/Desktop/IndustryLive_dev.ipa"
PGY_UPLOAD_RESULT=`curl -F "file=@$path" \
...
function JSONParse(){
    JSON_KEY="$1"
    echo  ${PGY_UPLOAD_RESULT} |python -c "import sys, json; print json.load(sys.stdin)['data']['$JSON_KEY']"|sed 's/[[:space:]]//g'
}
PGY_APP_NAME=$(JSONParse "buildName")
PGY_APP_VERSION=$(JSONParse "buildVersion")
PGY_APP_QRCODEURL=$(JSONParse "buildQRCodeURL")
PGY_APP_UPDATED=$(JSONParse "buildUpdated")
PGY_APP_IDENTIFIER=$(JSONParse "buildIdentifier")
PGY_APP_KEY=$(JSONParse "buildKey")
PGY_APP_BUILD_VERSION=$(JSONParse "buildBuildVersion")
